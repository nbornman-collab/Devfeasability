(function (global) {
  ...
  function deriveDevelopmentLogic(site) {
    ...
    var cityCore = site.borough === 'City of London' || !!site.inCAZ;
    var planning = site.planningSummary || site.planning || {};
    var heightIntent = !!(planning.heightIntent || (planning.liveTallCount || 0) > 0 || (planning.tallCount || 0) >= 2);
    var clusterPressure = !!(planning.clusterPressure || (planning.majorCount || 0) >= 2 || (planning.applicationCount || 0) >= 4);
    var tallCommercial = (cityCore && (existingHeight >= 35 || existingFloors >= 10) && /(office|commercial|mixed)/.test(useType || 'office')) || heightIntent;
    ...
    var type = tallCommercial ? 'Tower-on-podium'
      ...
      : 'Perimeter uplift';
    var posture = tallCommercial ? 'aggressive' : heritage === 'high' ? 'conservative' : 'balanced';
    return {
      mode: mode,
      type: type,
      posture: posture,
  ...
  function buildScenario(label, floors, metrics, upliftWeight) {
    ...
    var scoreLift = (metrics.logic.mode === 'Strategic height' ? 14 : metrics.logic.mode === 'Mid-rise intensification' ? 10 : 7) * upliftWeight;
  ...
  function buildCanonicalSite(site) {
    ...
    model.plotM2 = toNumber(site.plotM2, site.plot_m2, site.site_area, site.plot_area) || 0;
    model.existingHeightM = toNumber(site.existingHeightM, site.existing_height_m, site.heightM, site.height_m) || 0;
    model.existingFloors = inferExistingFloors(site, toNumber(site.floorToFloorM, 3.8) || 3.8);
    model.floorToFloorM = toNumber(site.floorToFloorM, 3.8) || 3.8;
